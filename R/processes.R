...
train_model_rf <- Process$new(
  id = "train_model_rf",
  description = "Takes a datacube and a set of training data and trains a random forest model.",
  categories = as.array("cubes"),
  summary = "Train a random forest model",
  parameters = list(
    Parameter$new(
      name = "ntree",
      description = "Number of trees to grow for the random forest model.",
      schema = list(
        type = "integer"
      )
    ),
    Parameter$new(
      name = "geojson",
      description = "GeoJSON file containing the training data.",
      schema = list(
        type = "object",
        subtype = "geojson"
      )
  ...
  returns = list(
    type = "object",
    description = "A trained random forest model."
  ),
  operation = function(cube, geojson, ntree){
    # load training data with sf library
    train_data <- sf::st_read(geojson, quiet = TRUE)
    ...
    extr <- gdalcubes::extract_geom(cube, train_data)
...
train_model_knn <- Process$new(
  ...
  returns = list(
    type = "object",
    description = "A trained k-nearest neighbor model."
  ),
  operation = function(cube, geojson, tuneLength){
    # load training data with sf library
    train_data <- sf::st_read(geojson, quiet = TRUE)
